...
func NewUserLoginLog(c *gin.Context) *userLoginLog {
	model := new(userLoginLog)
	model.TableName = "et_user_login_log"
	model.Ctx = c
	return model
}

func (this *userLoginLog) FetchByUserIdAndLimit(userId int, fields string, limit uint8, orderBy string) []UserLoginLogTable {
	if userId == 0 {
		return nil
	}
	this.InitEnjoythin()
	var rows []UserLoginLogTable
	pdo := this.Enjoythin.Table(this.TableName)
	if fields != "" {
